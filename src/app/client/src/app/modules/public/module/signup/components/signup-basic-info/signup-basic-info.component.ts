...
export class SignupBasicInfoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.personalInfoForm.get('country').setValue('India');
    this.personalInfoForm.get('country').valueChanges.subscribe(value => this.onCountryChange(value));

    console.log('Global Object data => ', this.startingForm);
  }

  onCountryChange(value: string) {
    this.isOtherCountrySelected = value === 'Others';
  
    if (this.isOtherCountrySelected) {
      this.personalInfoForm.get('state').disable();
      this.personalInfoForm.get('district').disable();
      this.personalInfoForm.get('state').setValue('NA');
      this.personalInfoForm.get('district').setValue('NA');
      this.personalInfoForm.get('otherCountry').setValidators([Validators.required]);
    } else {
      this.personalInfoForm.get('state').enable();
      this.personalInfoForm.get('district').enable();
      this.personalInfoForm.get('state').reset();
      this.personalInfoForm.get('district').reset();
      this.personalInfoForm.get('otherCountry').clearValidators();
      this.personalInfoForm.get('otherCountry').setValue('');
    }
  
    this.personalInfoForm.get('otherCountry').updateValueAndValidity();
  }
  ...
  next() {
    if (this.personalInfoForm.valid) {
      ...
      // userDetails.country = formValue?.country;
      ...
      if (formValue.country === 'Others') {
        userDetails.country = formValue.otherCountry;
        userDetails.state_id = 'NA';
        userDetails.state = 'NA';
        userDetails.district_id = 'NA';
        userDetails.district = 'NA';
      } else {
        userDetails.country = formValue.country;
